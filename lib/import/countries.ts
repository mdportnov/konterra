...
for (const [name, aliases] of COUNTRY_DATA) {
  lookup.set(name.toLowerCase(), name)
  for (const a of aliases) {
    lookup.set(a.toLowerCase(), name)
  }
}
...
export function isCityCountry(input: string): string | null {
  return CITY_COUNTRIES[input.toLowerCase().trim()] ?? null
}
